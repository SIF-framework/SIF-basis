...
REM Parameters that are used for multiple IMF-file objects
REM ------------------------------------------------------
REM CALSETi:        Name of calibration set i that is used for residual differences (both residual difference and sign are plotted)
REM HEADDIFFFILTER: Filter for head difference IDF-files to be shown in IMF-file
REM RESDIFFFILTER:  Filter for residual difference IPF-files to be shown in IMF-file
REM EFFECTPATH:     Path to effect directory under WORKOUT-folder, which should be processed
REM IMFFILENAME:    Specify result filename for IMF-file
SET CALSET1=PeilbuisDataXXX-KalSet
SET CALSET2=PeilbuisDataXXX-ValSet
SET HEADDIFFFILTER=diff*_L*.IDF
SET RESDIFFFILTER=*_L*.IPF
SET EFFECTPATH=%WORKOUTPATH%\%MODELREF:_=\%\effect
SET IMFFILENAME=%MODELREF% - modeleffects.IMF
...
SET IMODFILES="%DBASEPATH%\BASIS1\BND\IBOUND.IDF","%RESULTSPATH%\%MODELREF:_=\%\residu\*.IPF","%EFFECTPATH%\residu\%CALSET1%\split\dSGN\%RESDIFFFILTER%","%EFFECTPATH%\residu\%CALSET1%\split\%RESDIFFFILTER%"
SET IMODLEGENDS="%LEGENDPATH%\BND.leg","%LEGENDPATH%\residuen.leg","%LEGENDPATH%\dSGN.leg","%LEGENDPATH%\diff_residuen.leg"
SET CSLEGENDS="","","",""
SET FILESELECTIONS=0,0,1,1
SET EXTRAPARLINES="","COLUMN=-2;TEXTSIZE=5;THICKNESS=2","COLUMN=-1;TEXTSIZE=0;THICKNESS=3","COLUMN=-4;TEXTSIZE=5;THICKNESS=2"
IF DEFINED CALSET2 SET IMODFILES=%IMODFILES%,"%EFFECTPATH%\residu\%CALSET2%\split\dSGN\%RESDIFFFILTER%","%EFFECTPATH%\residu\%CALSET2%\split\%RESDIFFFILTER%"
IF DEFINED CALSET2 SET IMODLEGENDS=%IMODLEGENDS%,"%LEGENDPATH%\dSGN.leg","%LEGENDPATH%\diff_residuen.leg"
IF DEFINED CALSET2 SET CSLEGENDS=%CSLEGENDS%,"",""
IF DEFINED CALSET2 SET FILESELECTIONS=%FILESELECTIONS%,1,1
IF DEFINED CALSET2 SET EXTRAPARLINES=%EXTRAPARLINES%,"COLUMN=-1;TEXTSIZE=0;THICKNESS=3","COLUMN=-4;TEXTSIZE=5;THICKNESS=2"
...
SET REGISORDER=
SET REGISCOLORS=
SET MODELTOPBOTPATHS=
REM %DBASEPATH%\BASIS1\TOP;%DBASEPATH%\BASIS1\BOT
...
SET IMODFILES2="%DBASEPATH%\%MODELREF1%\MAAIVELD\*.IDF","%EFFECTPATH%\head\%HEADDIFFFILTER%", "%RESULTSPATH%\%MODELREF:_=\%\head\*_L*.IDF"
SET IMODLEGENDS2="%LEGENDPATH%\maaiveld_-10-100.leg","%LEGENDPATH%\diff_blauw-bruin_5cm-10m.leg","%LEGENDPATH%\maaiveld_-10-100.leg"
SET CSLEGENDS2=
SET FILESELECTIONS2=0,0,0
...
SET GENCOLORS=%GENCOLOR1%;%GENCOLOR2%;%GENCOLOR3%;%GENCOLOR4%;%GENCOLOR5%;100,100,100
