...
ECHO IPFFILE="%SOURCEPATH%\%SOURCEFILE%" >> %INIFILENAME%
IF NOT "%IXCOL%"=="" (
  ECHO IXCOL=%IXCOL% >> %INIFILENAME%
)
IF NOT "%IYCOL%"=="" (
  ECHO IYCOL=%IYCOL% >> %INIFILENAME%
)
IF NOT "%IZCOL%"=="" (
  ECHO IZCOL=%IZCOL% >> %INIFILENAME%
)
IF NOT "%CELLSIZE%"=="" (
...
IF NOT "%IDFNODATA%"=="" (
  ECHO NODATA=%IDFNODATA% >> %INIFILENAME%
)
ECHO IDFFILE=%RESULTPATH%\%RESULTFILE% >> %INIFILENAME%
ECHO GRIDFUNC=%GRIDFUNC% >> %INIFILENAME%
IF "%GRIDFUNC%"=="PERC" (
  ECHO PERCENTILE=%PERCENTILE% >> %INIFILENAME%
)
IF "%GRIDFUNC%"=="PCG" (
...
IF "%GRIDFUNC%"=="SKRIGING" (
  ECHO RANGE=%RANGE% >> %INIFILENAME%
  ECHO SILL=%SILL% >> %INIFILENAME%
  ECHO NUGGET=%NUGGET% >> %INIFILENAME%
  ECHO KTYPE=%KTYPE% >> %INIFILENAME%
  ECHO STDEVIDF=%STDEVIDF% >> %INIFILENAME% 
)
...
ECHO "%IMODEXE%" "%THISPATH%\%INIFILENAME%" >> %LOGFILE%
"%IMODEXE%" "%THISPATH%\%INIFILENAME%" >> %LOGFILE%
...
FOR %%G IN (%RESULTPATH%\%RESULTFILE%) DO (
  ECHO metadata maken voor %RESULTPATH%\%%~nG ...
