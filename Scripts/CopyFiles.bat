...
FOR /L %%i IN (0,1,%Nf%) DO (
  ...
  FOR %%G IN ("!SOURCEPATH!\!SOURCEFILTER!") DO (
    REM Ignore shortcuts and MET-files; MET-files are copied seperately in script.
    SET ISIGNORED=
    IF "%%~xG"==".lnk" SET ISIGNORED=1
    IF /I "%%~xG"==".MET" SET ISIGNORED=1
    IF NOT "!ISIGNORED!"=="1" (
        ...
        IF "!ISSKIPPED!" == "" (  
          ...
          IF "%ISADDMETADATA%"=="1" (
            ...
            IF EXIST "%SOURCEPATH%\!SOURCEFILEBASE!.MET" (
              IF "%ISMOVED%"=="1" (
                ECHO   moving metadata for "!SOURCEFILE!" ...
                ECHO   moving metadata for "!SOURCEFILE!" ... >> %LOGFILE%
                ...
                ECHO   copying metadata for "!SOURCEFILE!" ...
                ECHO   copying metadata for "!SOURCEFILE!" ... >> %LOGFILE%
            ...
            ECHO   "iMODmetadata.exe" !OVERWRITEMETADATA! "%TARGETPATH%\!TARGETFILEBASE!.%SOURCEFILEEXT%" "" %CURRENTDATE% 1 "%MODELREF0%" "%METADATA_DESCRIPTION%" "%CONTACTORG%" IDF "%METADATA_UNIT%" "" ="%SOURCEPATH%\!SOURCEFILE!; %METADATA_SOURCE%" "See !THISPATH:%ROOTPATH%\=!; %METADATA_DESCRIPTION%" ="%MODELREF0%" "%CONTACTORG%" "%CONTACTSITE%" "%CONTACTPERSON%" "%CONTACTEMAIL%" >> %LOGFILE%
            "%TOOLSPATH%\iMODmetadata.exe" !OVERWRITEMETADATA! "%TARGETPATH%\!TARGETFILEBASE!.%SOURCEFILEEXT%" "" %CURRENTDATE% 1 "%MODELREF0%" "%METADATA_DESCRIPTION%" "%CONTACTORG%" IDF "%METADATA_UNIT%" "" ="%SOURCEPATH%\!SOURCEFILE!; %METADATA_SOURCE%" "See !THISPATH:%ROOTPATH%\=!; %METADATA_DESCRIPTION%" ="%MODELREF0%" "%CONTACTORG%" "%CONTACTSITE%" "%CONTACTPERSON%" "%CONTACTEMAIL%" >> %LOGFILE%
            IF ERRORLEVEL 1 GOTO error
          )
        ) ELSE (
