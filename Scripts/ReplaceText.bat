...
IF "%ISFINDONLY%"=="1" (
  ...
  SET ACTION=replacing
)
SET RECURSIVE_TEXT=
IF "%ISRECURSIVE%"=="1" SET RECURSIVE_TEXT=(recursively) 
ECHO   Start %ACTION% text %RECURSIVE_TEXT%in '%SEARCHPATH%'
ECHO Start %ACTION% text %RECURSIVE_TEXT%in '%SEARCHPATH%' >> %LOGFILE%

REM Execute ReplaceText
ECHO "%REPLACETEXTEXE%" /m %SUMOPTION% %FINDOPTION% %CASEOPTION% %DATEOPTION% %REGEXPOPTION% %RECURSIVEOPTION% %BINOPTION% %EXCLUDEOPTION% "%SEARCHPATH%" "%FILTER%" %TEXT1% %TEXT2% >> %LOGFILE%
"%REPLACETEXTEXE%" /m %SUMOPTION% %FINDOPTION% %CASEOPTION% %DATEOPTION% %REGEXPOPTION% %RECURSIVEOPTION% %BINOPTION% %EXCLUDEOPTION% "%SEARCHPATH%" "%FILTER%" %TEXT1% %TEXT2% >> %LOGFILE%
